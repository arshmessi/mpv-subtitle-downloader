...
class SearchOrchestrator:
    def __init__(self, providers: Iterable[SubtitleProvider], timeout: float = 15.0, max_concurrency: int = 8) -> None:
    ...
    async def search(self, media: MediaInfo, languages: list[str] | None = None) -> SearchResponse:
        ...
        results = deduplicate([result for _, provider_results in collected for result in provider_results])
        rank_results(media, results, languages[0])
        for index, result in enumerate(results, 1):
            result.result_id = f"r{index}"
        return SearchResponse(bool(results), media, results, reports, None if results else "No suitable subtitles found")

    async def _search_provider(self, provider: SubtitleProvider, media: MediaInfo, languages: list[str]) -> tuple[ProviderReport, list[SubtitleResult]]:
        async with self._semaphore:
            try:
                response = await asyncio.wait_for(provider.search(media, languages), self.timeout)
                return ProviderReport(provider.id, ProviderStatus.OK, len(response.results), response.message), response.results
            except asyncio.TimeoutError:
                return ProviderReport(provider.id, ProviderStatus.TIMEOUT, message="provider timed out"), []
            except PermissionError as error:
                return ProviderReport(provider.id, ProviderStatus.AUTHENTICATION_REQUIRED, message=str(error)), []
