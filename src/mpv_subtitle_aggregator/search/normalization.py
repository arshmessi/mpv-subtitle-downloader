...
def deduplicate(results: list[SubtitleResult]) -> list[SubtitleResult]:
    ...
    for result in results:
        key = (result.content_hash or _normalize(result.release), result.language.lower(), result.hearing_impaired, result.forced)
        existing = grouped.get(key)
        if existing is None:
            grouped[key] = result
        else:
            existing.providers = sorted(set(existing.providers + result.providers + [result.provider]))
